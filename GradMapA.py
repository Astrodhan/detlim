...
# -*- coding: utf-8 -*-
"""
Created on Sat May 22 15:27:37 2021
...
class pgm:                                                                      #pgm for periodogram
    ...
    def FT(f,mp,d):                                                             # Resturns the power values of the Fourier Transform of input signal+ added sine
        rv1=rv+pgm.A(1047.94,mp,f)*np.sin(2*np.pi*times*f+d)                     #Adding a simulated signal to our RV values, 1047 is solar mass in terms of jupiter masses
...
fakemass, fakefreq=np.meshgrid(fakemass, fakefreq)
rows, cols = (xyres, xyres) 
prob = [[0 for i in range(cols)] for j in range(rows)] # Detection Probability Matrix
...
for i in range(xyres):
    for j in range(xyres):
        amp[i][j]=pgm.A(1047.94,fakemass[j][j],fakefreq[i][i])

def detprob(m,n):                               # Indices are inputs Calculates the fraction of phase for which the periodogram signal corresponding to the added signal exceeds the False Alarm Probability
    ...
    for i in range(0,res):
        if farray[i]>fakefreq[n][n]-bandwidth and farray[i]<fakefreq[n][n]+bandwidth:
            fband.append(i)                     # i is appended and not farray[i] because the power array follows the same indexing.
     
    for i in range(zres):
        rv1=rv+amp[m][n]*np.sin(2*np.pi*times*fakefreq[n][n]+i*2*np.pi/zres)                     #Adding a simulated signal to our RV values, 1047 is solar mass in terms of jupiter masses
        gls2=pyPeriod.Gls((times,rv1,error),fbeg=f_min,fend=f_max,freq=farray)
        powar=gls2.power
        for j in fband:
...
def execute():    
    for j in range(xyres):
        for i in range(xyres):
            prob[i][j]=detprob(i,j)[0]
            peakpow[i][j]=detprob(i,j)[1]
